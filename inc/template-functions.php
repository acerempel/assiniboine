...
function assinboine_html_classes($classes) {
  // Use EB Garamond font. TODO: make this configurable.
  $classes .= ' garamond ';
  return $classes;
}

add_filter( 'twentytwentyone_html_classes', 'assinboine_html_classes' );
...
function twenty_twenty_one_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<p class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . twenty_twenty_one_continue_reading_text() . '</a></p>';
	}
}
 ...
 * @return string
 */
function twenty_twenty_one_get_icon_svg( $group, $icon, $size = 24, $classes = '' ) {
	return Twenty_Twenty_One_SVG_Icons::get_svg( $group, $icon, $size, $classes);
}
